...
def transform_to_io(df_purchaser):
    ...
    return df_io, df_a, df_value_add, df_import, df_indirect_tax, df_final_demand
...
if __name__ == '__main__':
    ...
    # Load data
    raw_df = load_data(file_name = "DataIO2010x16.xlsx")
    ...
    # Transform to IO table by purchaser
    df_io, df_a, df_value_add, df_import, df_indirect_tax, df_final_demand = transform_to_io(df_purchaser)

    identity_array = np.identity(len(df_a)) * 1
    df_sum = pd.DataFrame(df_io.sum(axis = 1)[0:-11]).transpose()
    for col in df_a.columns:
        df_a[col] = df_a[col]/df_sum[col][0]
    df_leontief = pd.DataFrame(inv(identity_array - df_a.to_numpy()),columns = df_a.columns,index = df_a.index)
    a = 3